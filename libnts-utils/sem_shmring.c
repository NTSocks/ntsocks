 ...
 */
static inline sem_shmring_handle_t
_sem_shmring_init(char *shm_addr, size_t addrlen,
                  size_t ele_size, size_t ele_num, bool is_owner)
{
    assert(shm_addr);
    assert(addrlen > 0);
    assert(ele_size > 0);
    ...
    handle->addrlen = addrlen + 1;
    memcpy(handle->shm_addr, shm_addr, addrlen);
    handle->ele_size = ele_size;
    handle->capacity = ele_num;
    handle->queue_size =
        sizeof(struct sem_shmring_buf) + ele_size * ele_num;

    sprintf(handle->mutex_sem_name,
            "%s%s", SEM_MUTEX_PREFIX, shm_addr);
    sprintf(handle->buf_count_sem_name,
            "%s%s", SEM_BUF_COUNT_PREFIX, shm_addr);
    sprintf(handle->spool_signal_sem_name,
            "%s%s", SEM_SPOOL_SIGNAL_PREFIX, shm_addr);

    handle->data = (char **)
        calloc(handle->capacity, sizeof(char *));
    if (!handle->data)
    {
    ...
    if (UNLIKELY(handle->shm_fd == -1))
    {
        ...
        {
            handle->shm_fd = shm_open(
                handle->shm_addr, shm_mask, shm_mode);
            if (handle->shm_fd == -1)
            {
                perror("shm_open");
    ...
    handle->queue = (circular_queue_t)mmap(NULL,
                                           handle->queue_size, PROT_READ | PROT_WRITE,
    ...
    handle->queue->data_area =
        (uint8_t *)((uint8_t *)handle->queue + sizeof(struct sem_shmring_buf));
    for (int i = 0; i < handle->capacity; i++)
    {
        handle->data[i] = (char *)(handle->queue->data_area + i * handle->ele_size);
    }

    // store old, umask for world-writable access of
    //  semaphores (mutex_sem, buf_count_sem, spool_signal_sem)
    mode_t old_umask;
    old_umask = umask(0);

    // mutual exclusion semaphore, mutex_sem with an initial value 0.
    handle->mutex_sem = sem_open(
        handle->mutex_sem_name, O_CREAT, 0666, 0);
    ...
    // counting semaphore, indicating the number of 
    //  available buffers in shm ring.
    // init value = NTM_MAX_BUFS
    handle->buf_count_sem = sem_open(
        handle->buf_count_sem_name, O_CREAT, 0666, handle->capacity);
    ...
    // count semaphore, indicating the number 
    //  of data buffers to be read. Init value = 0
    handle->spool_signal_sem = sem_open(
        handle->spool_signal_sem_name, O_CREAT, 0666, 0);
...
sem_shmring_handle_t
sem_shmring_create(char *shm_addr,
                   size_t addrlen, size_t ele_size, size_t ele_num)
{
    return _sem_shmring_init(
        shm_addr, addrlen, ele_size, ele_num, true);
}

// When slave first try to init/connect existing sem_shmring
sem_shmring_handle_t
sem_shmring_init(char *shm_addr,
                 size_t addrlen, size_t ele_size, size_t ele_num)
{
    return _sem_shmring_init(
        shm_addr, addrlen, ele_size, ele_num, false);
}

// Returns 0 on success, -1 if buffer is full
int sem_shmring_push(
    sem_shmring_handle_t handle,
    char *element, size_t ele_len)
{
    ...
    handle->queue->write_idx =
        next_index(handle->queue->write_idx, handle->capacity);
...
int sem_shmring_pop(sem_shmring_handle_t handle, char *element, size_t ele_len)
{
    ...
    DEBUG("write_idx=%ld, read_idx=%ld",
          handle->queue->write_idx, handle->queue->read_idx);
    memcpy(element,
           handle->data[handle->queue->read_idx], ele_len);

    handle->queue->is_full = 0;
    handle->queue->read_idx =
        next_index(handle->queue->read_idx, handle->capacity);
...
// Free sem_shmring structure, 
//  if unlink is 1, unlink all SHM-based variables.
int sem_shmring_free(
    sem_shmring_handle_t handle, bool is_unlink)
{
    assert(handle);
...
{
    assert(handle);
    return (!handle->queue->is_full &&
            empty(handle->queue->write_idx, handle->queue->read_idx));
}
