...
nts_context_t get_nts_context() { return NULL; }
...
int nts_context_init(const char *config_file)
{
   ...
   */
    ...
    nts_ctx->ntm_ctx =
        (nts_ntm_context_t)calloc(1, sizeof(struct nts_ntm_context));
    if (!nts_ctx->ntm_ctx)
    {
        perror(err_msg);
    ...
    nts_ctx->ntp_ctx =
        (nts_ntp_context_t)calloc(1, sizeof(struct nts_ntp_context));
    if (!nts_ctx->ntp_ctx)
    {
        perror(err_msg);
    ...
    /**
   * init hash map for the mapping between epoll nt_socket_id and
   * nt_epoll_context_t
   */
   ...
   */
    ...
    ret = ntm_shm_connect(ntm_ctx->shm_send_ctx, NTM_SHMRING_NAME,
                          sizeof(NTM_SHMRING_NAME));
    if (ret)
    {
...
void nts_context_destroy()
{
   ...
   */
    ...
    /**
   * destroy the nt_socket-level nt_sock_context_t for each established
   * nt_socket
   */
     ...
     * TODO: 0. update local socket state, notify the ntm to destroy nt_socket
     * resources
     * TODO: 1. close/destroy the send/recv ntp shm ring queue
     * 2. close/destroy the nts shm ring queue between nt_socket and ntm
     */
        DEBUG("0. update local socket state, notify the ntm to destroy nt_socket "
              "resources");
    ...
    while (hasNextHashMapIterator(iter))
    {
         ...
         * TODO:0. update local epoll state, notify the ntm/ntp to destroy epoll
         * resources
         * TODO:1. close/destroy the epoll-corresponding shm event queue;
         * 2. close/destroy the epoll-corresponding shm ring queue between libnts
         * and ntm.
         */
