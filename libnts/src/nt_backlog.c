...
_backlog_init(nt_socket_t listener, char *shm_addr,
              size_t addrlen, int backlog_size, bool is_owner)
{
    ...
    backlog_ctx = calloc(
        1, sizeof(struct nt_backlog_context));
    ...
    backlog_ctx->backlog_size =
        backlog_size <= 0 ? BACKLOG_SIZE : backlog_size;
    ...
    if (is_owner)
    {
        backlog_ctx->shmring_handle =
            nt_spsc_shmring_init(backlog_ctx->shm_addr,
                                 addrlen, backlog_ctx->backlog_size, NT_SOCK_SIZE);
    }
    else
    {
        backlog_ctx->shmring_handle =
            nt_get_spsc_shmring(backlog_ctx->shm_addr,
                                addrlen, backlog_ctx->backlog_size, NT_SOCK_SIZE);
    }
...
backlog_nts(nt_socket_t listener,
            char *shm_addr, size_t addrlen, int backlog_size)
{
    return _backlog_init(
        listener, shm_addr, addrlen, backlog_size, false);
}

nt_backlog_context_t
backlog_ntm(nt_socket_t listener,
            char *shm_addr, size_t addrlen, int backlog_size)
{
    return _backlog_init(
        listener, shm_addr, addrlen, backlog_size, true);
}

int backlog_push(
    nt_backlog_context_t backlog_ctx, nt_socket_t socket)
{
    assert(backlog_ctx);
    assert(socket);
    ...
    ret = nt_spsc_shmring_push(
        backlog_ctx->shmring_handle,
        (char *)&backlog_sock, NT_SOCK_SIZE);
