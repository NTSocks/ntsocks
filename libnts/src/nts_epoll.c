...
int nts_close_epoll_socket(int epid)
{
    ...
    nt_epoll_context_t epoll_ctx =
        (nt_epoll_context_t)Get(nts_ctx->nt_epoll_map, &epid);
    if (!epoll_ctx)
    {
        errno = EINVAL;
     ...
     * 2. instruct ntm to remove the epoll of all listen socket 
     *      within the specific epoll_socket
     * 3. [backend] ntm instruct ntp to remove the epoll 
     *      of all client socket within the specific epoll_socket
    ...
    // 2. instruct ntm to remove the epoll of all
    //      listen socket within the specific epoll_socket
    ntm_msg req_msg;
...
int nts_epoll_create(int size)
{
    ...
    if (req_msg.msg_id != resp_msg.id || resp_msg.retval == -1 || resp_msg.shm_addrlen <= 0)
    {
...
int nts_epoll_ctl(int epid, int op, int sockid, nts_epoll_event *event)
{
    ...
    if (op == NTS_EPOLL_CTL_ADD)
    {
        ...
        // parse the response epoll_msg
        if (req_msg.msg_id != resp_msg.id || resp_msg.retval == -1)
        {
            ERR("invalid message id or retval == -1 for NTM_MSG_EPOLL_CREATE response");
            errno = ENFILE;
            return -1;
...
int nts_epoll_wait(int epid, nts_epoll_event *events, int maxevents, int timeout)
{
    ...
    do
    {
        ...
        while (eq->num_events == 0 && timeout != 0)
        {
            ep->stat.waits++;
            ep->waiting = 1;
        ...
        {
            nt_sock_context_t event_sock_ctx = (nt_sock_context_t)
                Get(nts_ctx->nt_sock_map, &eq_ctx->events[eq->start].sockid);
