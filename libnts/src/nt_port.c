...
int init_port_context(
    nt_port_context_t nt_port_ctx, int max_port)
{
    int i;
    if (max_port <= 0)
    {
    ...
    nt_port_ctx->ntport = (nt_port_t)
        calloc(max_port, sizeof(struct nt_port));
...
nt_port_t allocate_port(
    nt_port_context_t nt_port_ctx, int need_lock)
{
    nt_port_t port = NULL;
    ...
    nt_port_context_t nt_port_ctx, int portid, int need_lock)
{
    ...
    TAILQ_INSERT_TAIL(
        &nt_port_ctx->free_ntport, port, free_ntport_link);
    if (need_lock)
    {
        pthread_mutex_unlock(&nt_port_ctx->port_lock);
    }
...
nt_port_t get_port(
    nt_port_context_t nt_port_ctx, int portid, int max_port)
{
