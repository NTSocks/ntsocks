...
typedef struct
{
    unsigned int column;
    int node_idx;
    uint64_t shm_offset; // point to the pre-applied shm data memory segment
    int next_node_idx;   // indicate the index of next shm_mempool_node
...
shm_mp_handler_t
shm_mp_init(unsigned int block_len,
            unsigned int block_count, char *shm_name, size_t shm_name_len);

shm_mempool_node *
shm_mp_malloc(shm_mp_handler_t mp_handler, unsigned int size);

shm_mempool_node *
shm_mp_node(shm_mp_handler_t mp_handler, unsigned int node_idx);
