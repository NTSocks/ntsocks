 ...
 * @return
 */
ntm_shmring_handle_t
ntm_shmring_init(char *shm_addr, size_t addrlen)
{
    assert(shm_addr);
    assert(addrlen > 0);
    ...
    shmring_handle = (ntm_shmring_handle_t)
        malloc(sizeof(ntm_shmring_t));
    memset(shmring_handle, 0, sizeof(ntm_shmring_t));
    ...
    shmring_handle->shm_fd =
        shm_open(NTM_SHM_NAME, O_RDWR | O_CREAT, 0666);
    ...
    int ret;
    ret = ftruncate(shmring_handle->shm_fd,
                    sizeof(struct ntm_shmring_buf));
    if (ret == -1)
    {
        error("ftruncate");
        goto FAIL;
    ...
    //  mutex_sem with an initial value 0.
    shmring_handle->mutex_sem =
        sem_open(NTM_SEM_MUTEX_NAME, O_CREAT, 0666, 0);
    ...
    // counting semaphore, indicating the number
    //  of available buffers in shm ring.
    // init value = NTM_MAX_BUFS
    shmring_handle->buf_count_sem =
        sem_open(NTM_SEM_BUF_COUNT_NAME, O_CREAT, 0666, NTM_MAX_BUFS);
    ...
    shmring_handle->spool_signal_sem =
        sem_open(NTM_SEM_SPOOL_SIGNAL_NAME, O_CREAT, 0666, 0);
...
ntm_shmring_handle_t
ntm_get_shmring(char *shm_addr, size_t addrlen)
{
    assert(shm_addr);
    assert(addrlen > 0);
    ...
    shmring_handle->shmring = (struct ntm_shmring_buf *)
        mmap(NULL, sizeof(struct ntm_shmring_buf),
             PROT_READ | PROT_WRITE, MAP_SHARED, shmring_handle->shm_fd, 0);
    if (shmring_handle->shmring == MAP_FAILED)
    {
        error("mmap");
        goto FAIL;
    ...
    // store old, umask for world-writable access
    //  of semaphores (mutex_sem, buf_count_sem, spool_signal_sem)
    mode_t old_umask = umask(0);
    ...
    // count semaphore. indicate the number
    //  of available shm segments to be written.
    shmring_handle->buf_count_sem =
        sem_open(NTM_SEM_BUF_COUNT_NAME, O_CREAT, 0666, NTM_MAX_BUFS);
    if (shmring_handle->buf_count_sem == SEM_FAILED)
    {
 ...
 * @return
 */
bool ntm_shmring_push(
    ntm_shmring_handle_t self, ntm_msg *element)
{
    assert(self);
    assert(element);
    int rc;
    ...
    /* Critical Section */
    DEBUG("ntm_write_index=%d, ntm_read_index=%d",
    ...
    DEBUG("After push, ntm_write_index=%d, ntm_read_index=%d",
          self->shmring->write_index, self->shmring->read_index);
    ...
    // tell spooler that there is a element
    //  to read: V (spool_signal_sem)
    ...
    DEBUG("push ntm shmring success!");
    ...
    ntm_shmring_handle_t self, ntm_msg *element)
{
    ...
    ntm_msgcopy(
        &(self->shmring->buf[self->shmring->read_index]), element);
    ...
    {
        self->shmring->write_index =
            self->shmring->write_index - NTM_MAX_BUFS;
    }
