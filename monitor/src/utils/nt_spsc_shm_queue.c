 ...
 * @param element_num
 * @param element_size
 * @param is_master : if it is invoked by nt_spsc_shmring_init, 
 *                      `is_master` is true; else, `is_master` is false
 * @return
 */
...
nt_spsc_shmring_init(char *shm_addr, size_t addrlen,
                     int element_num, size_t element_size)
{
 ...
 */
nt_spsc_shmring_handle_t
nt_get_spsc_shmring(char *shm_addr, size_t addrlen,
                    int element_num, size_t element_size)
{
    return _nt_spsc_shmring_init(
    ...
    nt_spsc_shmring_handle_t self, char *element, size_t element_size)
{
    ...
    element_size = (element_size > self->element_size)
                       ? self->element_size
                       : element_size;
    ...
    nt_atomic_store64_explicit(self->read_index,
