...
#define IPADDR_LEN 16
	struct nts_shm_conn
	{
		...
		// if nt_socket type is created by `accept()`, 
		//	point to coresponding `nt_listener_wrapper_t`
	...
	/*----------------------------------------------------------------------------*/
	/**
	 * Definitions for the shm communication 
	 * 	between nt-monitor and ntb-proxy.
	 */
	...
	/*----------------------------------------------------------------------------*/
	/**
	 * Definitions for the socket communication 
	 * 	between local ntb-monitor and remote ntb-monitor.
	 */
	struct ntm_conn
	{
		char ip[16];
...
#define BACKLOG_SHMLEN 30
	struct nt_listener_wrapper
	{
		...
		// backlog shm name, default rule: backlog-{sock_id}
		char backlog_shmaddr[BACKLOG_SHMLEN]; 
