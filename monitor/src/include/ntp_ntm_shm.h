...
extern "C"
{
	...
	/**
	 * used by nt-monitor to disconnect 
	 * 	the shm-ring based connection with libnts app
	 */
	int ntp_ntm_shm_ntm_close(ntp_ntm_shm_context_t shm_ctx);

	/**
	 * used by libnts app or nt-monitor to 
	 * 	free the memory of nts_shm_context
	 */
