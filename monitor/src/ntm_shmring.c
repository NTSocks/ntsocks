...
ntm_shmring_handle_t ntm_shmring_init(char *shm_addr, size_t addrlen)
{
    ...
    shmring_handle->buf_count_sem = sem_open(
        NTM_SEM_BUF_COUNT_NAME, O_CREAT, 0666, NTM_MAX_BUFS);
    ...
    shmring_handle->spool_signal_sem = sem_open(
        NTM_SEM_SPOOL_SIGNAL_NAME, O_CREAT, 0666, 0);
...
bool ntm_shmring_pop(ntm_shmring_handle_t self, ntm_msg *element)
{
    ...
    DEBUG("ntm_write_index=%ld, ntm_read_index=%ld",
