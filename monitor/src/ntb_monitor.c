...
static inline bool
try_close_ntm_listener(ntm_conn_ctx_t ntm_conn_ctx);

/**
 * destroy nt_socket-level nt_socket-related resources, 
 * 	i.e., nts_shm_conn, nts_shm_ctx, nt_socket_id, nt_port
 * 		nts_shm_context, accepted_conn_map, port_sock_map, 
 * 		nts_shm_conn_map(ntm_nts_context)
 */
static inline void
destroy_client_nt_socket_conn(
	nt_socket_t client_socket, nts_shm_conn_t nts_shm_conn, int bound_port);

static inline void
destroy_client_nt_socket_conn_with_sock(nt_socket_t client_socket);

static inline void
destroy_client_nt_socket_conn_with_nts_shm_conn(nts_shm_conn_t nts_shm_conn);
...
int ntm_init(const char *config_file)
{
	...
	ntm_mgr->nt_sock_ctx =
		(nt_sock_context_t)calloc(1, sizeof(struct nt_sock_context));
	...
	ntm_mgr->nt_port_ctx =
		(nt_port_context_t)calloc(1, sizeof(struct nt_port_context));
	...
	// hash map for the mapping relationship
	//	between port and client nt_socket
	ntm_mgr->port_sock_map = createHashMap(NULL, NULL);

	ntm_mgr->nts_ctx =
		(ntm_nts_context_t)calloc(1, sizeof(struct ntm_nts_context));
	if (!ntm_mgr->nts_ctx)
	{
		perror(err_msg);
	...
	ntm_mgr->ntp_ctx =
		(ntm_ntp_context_t)calloc(1, sizeof(struct ntm_ntp_context));
	if (!ntm_mgr->ntp_ctx)
	{
		perror(err_msg);
	...
	ntm_mgr->ntm_conn_ctx =
		(ntm_conn_ctx_t)calloc(1, sizeof(struct ntm_conn_context));
	...
	ntm_mgr->nt_listener_ctx =
		(nt_listener_context_t)calloc(1, sizeof(struct nt_listener_context));
	...
	/**
	 * init the ntp-related shm ringbuffer queue 
	 * 		to recv/send from/to ntb-proxy
	...
	if (!ntp_ep_send_ctx)
	{
		perror(err_msg);
		ERR("Failed to \
allocate memory for 'epoll_sem_shm_ctx_t ntp_ep_send_ctx'.");
		goto FAIL;
	}
	ret = epoll_sem_shm_connect(
		ntp_ep_send_ctx, NTP_EP_SEND_QUEUE, strlen(NTP_EP_SEND_QUEUE) + 1);
	if (ret != 0)
	{
		ERR("Failed to \
epoll_sem_shm_connect for 'epoll_sem_shm_ctx_t ntp_ep_send_ctx'.");
		goto FAIL;
	}
	...
	if (!ntp_ep_recv_ctx)
	{
		perror(err_msg);
		ERR("Failed to \
allocate memory for 'epoll_sem_shm_ctx_t ntp_ep_recv_ctx'.");
		goto FAIL;
	}
	ret = epoll_sem_shm_connect(
		ntp_ep_recv_ctx, NTP_EP_RECV_QUEUE, strlen(NTP_EP_RECV_QUEUE) + 1);
	if (ret != 0)
	{
		ERR("Failed to \
epoll_sem_shm_connect for 'epoll_sem_shm_ctx_t ntp_ep_recv_ctx'.");
		goto FAIL;
	}
...
static inline void
handle_nt_syn_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg);
static inline void
handle_nt_syn_ack_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg);
static inline int
handle_nt_client_syn_ack_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg);
static inline void
handle_nt_invalid_port_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg);
static inline void
handle_nt_listener_not_found_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg);
static inline void
handle_nt_listener_not_ready_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg);
static inline void
handle_backlog_is_full_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg);
...
inline void handle_nt_syn_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{
	...
	memset(&outgoing_msg, 0, sizeof(ntm_sock_msg));
	...
	listener_wrapper =
		(nt_listener_wrapper_t)Get(ntm_mgr->nt_listener_ctx->listener_map, &dport);

	DEBUG("gain nt_listener according to dport[%d] with nt_socket state %d",
		  dport, listener_wrapper->listener->socket->state);
	...
	{
		sched_yield();
		retval = ntp_ntm_shm_recv(
			ntm_mgr->ntp_ctx->shm_recv_ctx, &ntp_incoming_msg);
	}

	INFO("ntp_incoming_msg.msg_type=%d, msg info=%s",
		 ntp_incoming_msg.msg_type, ntp_incoming_msg.msg);
	...
	DEBUG("recv the response message \
of `ntb connection setup successfully` from ntp.");
	...
	// push the new `client_socket` into
	//	coresponding `listener_wrapper->accepted_conn_map`
	Put(listener_wrapper->accepted_conn_map,
		&client_socket->sockid, client_socket);
	...
	if (listen_conn->socket->epoll &&
		(listen_conn->socket->epoll & NTS_EPOLLIN))
	{
	...
	// set the allocated nt_port sin_port of
	//	accepted nt_socket as source port in `outgoing_msg.sport`
...
inline void handle_nt_syn_ack_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{
	...
	DEBUG("recv the response \
message of `ntb connection setup successfully` from ntp.");
	...
	memset(&resp_sock_msg, 0, sizeof(ntm_sock_msg));
	...
	{
		ERR("The nts_shm_conn of client socket[sockid=%d, port=%d] is not found",
			(int)client_sock->sockid, dport);
		return;
	}
	nts_shm_conn_t nts_shm_conn;
	nts_shm_conn = (nts_shm_conn_t)Get(
		ntm_mgr->nts_ctx->nts_shm_conn_map, &client_sock->sockid);
...
handle_nt_client_syn_ack_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{
	...
	*	 ii. forward `CLIENT_SYN_ACK` nts_msg into 
				corresponding client nt_socket via nts_shm
	* 
	*/
	assert(msg.sockid > 0);
	int retval = 1;
	...
	nt_listener_wrapper_t listener_wrapper;
	listener_wrapper = (nt_listener_wrapper_t)Get(
		ntm_mgr->nt_listener_ctx->listener_map, &dport);
	DEBUG("gain nt_listener \
according to dport[%d] with nt_socket state %d",
		  dport, listener_wrapper->listener->socket->state);
	if (listener_wrapper->listener->socket->state != LISTENING)
	{
		outgoing_msg.type = NT_LISTENER_NOT_READY;
	...
	// 	 i. update corresponding client nt_socket state as `ESTABLISHED`
	// 	 ii. forward `CLIENT_SYN_ACK` nts_msg into
	//			corresponding server listening nt_socket via nts_shm
	nt_socket_t client_socket;
	client_socket = (nt_socket_t)Get(
		listener_wrapper->accepted_conn_map, &msg.sockid);
	...
	resp_msg.retval = 0;
	retval = nts_shm_send(
		listener_wrapper->nts_shm_conn->nts_shm_ctx, &resp_msg);
	if (retval)
	{
...
inline void
handle_nt_fin_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{
	DEBUG("handle NT_FIN success");
}

inline void
handle_nt_fin_ack_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{

	/**
	 * 1. locate the local coressponding nt_socket in 
	 * 		`ntm_mgr->port_sock_map` according to the `<dip, dport, sip, sport>` 
	 * 2. get `nts_shm_conn_t` according to nt_socket, 
	 * 		update nt_socket state to `WAIT_CLOSE`
	...
	if (!Exists(ntm_mgr->nts_ctx->nts_shm_conn_map, &client_sock->sockid))
	{
		ERR("The nts_shm_conn of \
client socket[sockid=%d, port=%d] is not found",
			(int)client_sock->sockid, dport);
		goto FAIL;
	}
	nts_shm_conn_t nts_shm_conn;
	...
	/**
	 * 4. destroy nt_socket-related resources, 
	 * 		i.e., nts_shm_conn, nts_shm_ctx, nt_socket_id, nt_port
	 * 		nts_shm_context, accepted_conn_map, 
	 * 		port_sock_map, nts_shm_conn_map(ntm_nts_context)
	 */
...
inline void
handle_connect_ok_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{
	DEBUG("handle_connect_ok_msg success");
}

inline void
handle_failure_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{
	DEBUG("handle_failure_msg success");
}

inline void
handle_stop_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{
...
inline void
handle_stop_confirm_msg(ntm_conn_t ntm_conn, ntm_sock_msg msg)
{
...
void *ntm_sock_listen_thread(void *args)
{
	...
	// begin listening
	ntm_start_tcp_server(ntm_conn_ctx->server_sock,
...
inline void handle_msg_nts_new_socket(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	...
	new_nts_shm_conn->socket =
		allocate_socket(ntm_mgr->nt_sock_ctx, msg.sock_type, 1);
	new_nts_shm_conn->sockid = new_nts_shm_conn->socket->sockid;
	...
	if (msg.nts_shm_addrlen > 0)
	{
		...
		memcpy(new_nts_shm_conn->nts_shm_name,
			   msg.nts_shm_name, msg.nts_shm_addrlen);
	}

	new_nts_shm_conn->nts_shm_ctx = nts_shm();
	retval = nts_shm_connect(new_nts_shm_conn->nts_shm_ctx,
							 msg.nts_shm_name, msg.nts_shm_addrlen);
	if (retval)
	{
	...
	// push new nts shm conn into hashmap
	Put(ntm_mgr->nts_ctx->nts_shm_conn_map,
		&new_nts_shm_conn->socket->sockid, new_nts_shm_conn);
	DEBUG("push new nts shm conn into hashmap with sockid=%d, new_nts_shm_conn=%p.",
		  new_nts_shm_conn->socket->sockid, new_nts_shm_conn);
...
FAIL:
	...
	if (new_nts_shm_conn)
	{
		...
		{
			free_socket(ntm_mgr->nt_sock_ctx,
						new_nts_shm_conn->socket->sockid, 1);
		}
 ...
 */
inline void
handle_msg_nts_connect(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	 ...
	 * 	 4. send `ntb channel setup` message into ntb-proxy 
	 * 		to setup the NTB connection/queue
	 * 	 5. receive `setup ready` message<shm name of 
	 * 		 send/recv queue for the communication
	...
	if (nts_shm_conn->socket->state != CLOSED && nts_shm_conn->socket->state != BOUND)
	{
		...
		if (retval)
		{
			ERR("nts_shm_send failed \
for response to NT_ERR_REQUIRE_CLOSED_OR_BOUND_FIRST");
		}
		goto FAIL;
	}
	...
	DEBUG("ready to connect remote ntb-based server with %s:%d, addrlen=%d",
		  nts_shm_conn->ip, msg.port, msg.addrlen);

	/**
	 * Check whether the communication between local 
	 * 	and remote ntm is setup (ntm_conn_context). 
	 * If the communication between local ntm 
	 * 	and remote IP-specified ntm is not setup,
	...
	else
	{
		...
		if (retval)
		{
			...
			retval = nts_shm_send(
				nts_shm_conn->nts_shm_ctx, &response_msg);
			if (retval)
			{
	...
	memset(&syn_msg, 0, sizeof(ntm_sock_msg));
...
inline void handle_msg_nts_bind(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	...
	DEBUG("handle_msg_nts_bind: sockid=%d, ip_addr=%s, port=%d",
		  msg.sockid, msg.address, msg.port);
	response_msg.msg_id = msg.msg_id;
	response_msg.msg_type = NTS_MSG_BIND;
	response_msg.sockid = msg.sockid;
	nts_shm_conn = (nts_shm_conn_t)Get(
		ntm_mgr->nts_ctx->nts_shm_conn_map, &msg.sockid);
	...
	if (msg.port < 0 ||
		msg.port >= ntm_mgr->nt_port_ctx->num_ports) // or other invalid range
	{
		response_msg.retval = -1;
		response_msg.nt_errno = NT_ERR_INVALID_PORT;
		retval = nts_shm_send(nts_shm_conn->nts_shm_ctx, &response_msg);
	 ...
	 * 3. put the nt_listener_wrapper into 
	 * 		`listener_map`(hash map) of `nt_listener_context`
	 */
	nts_shm_conn->port = msg.port;
	nts_shm_conn->addrlen = msg.addrlen;
	memcpy(nts_shm_conn->ip, msg.address, msg.addrlen);
...
inline void handle_msg_nts_listen(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	...
	new_listener_wrapper = (nt_listener_wrapper_t)
		calloc(1, sizeof(struct nt_listener_wrapper));
	...
	memcpy(new_listener_wrapper->ip,
		   nts_shm_conn->ip, nts_shm_conn->addrlen);
	...
	Put(ntm_mgr->nt_listener_ctx->listener_map,
		&new_listener_wrapper->port, new_listener_wrapper);
	DEBUG("Put new_listener_wrapper \
into `listener_map` indexed by nt_port=%d success",
		  new_listener_wrapper->port);
...
inline void
handle_msg_nts_accept(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	// assert(msg.msg_id > 0);
	// assert(msg.sockid > 0);
...
inline void
handle_msg_nts_accept_ack(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	nts_shm_conn_t nts_shm_conn;
	...
	if (msg.nts_shm_addrlen > 0)
	{
		...
		memcpy(nts_shm_conn->nts_shm_name,
			   msg.nts_shm_name, msg.nts_shm_addrlen);
	}

	int retval;
	nts_shm_conn->nts_shm_ctx = nts_shm();
	retval = nts_shm_connect(nts_shm_conn->nts_shm_ctx,
							 msg.nts_shm_name, msg.nts_shm_addrlen);
	if (retval == -1)
	{
		ERR("new nts_shm_conn connect nts shm ringbuffer failed. ");
...
inline void
handle_msg_nts_close(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	...
	free(listener_wrapper->accepted_conn_map);
	listener_wrapper->accepted_conn_map = NULL;
...
inline void
handle_msg_nts_fin(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	...
	memset(&sock_msg, 0, sizeof(ntm_sock_msg));
...
inline void
handle_msg_nts_epoll_create(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	...
	memcpy(epoll_ctx->epoll_shmaddr,
		   msg.nts_shm_name, epoll_ctx->epoll_shmlen);
	...
	memcpy(req_ep_msg.shm_name,
		   epoll_ctx->io_queue_shmaddr, epoll_ctx->io_queue_shmlen);

	retval = epoll_sem_shm_send(
		ntm_mgr->epoll_mgr->ntp_ep_recv_ctx, &req_ep_msg);
	if (retval)
	{
	...
	retval = epoll_sem_shm_recv(
		ntm_mgr->epoll_mgr->ntp_ep_send_ctx, &resp_ep_msg);
...
inline void handle_msg_nts_epoll_ctl(ntm_manager_t ntm_mgr, ntm_msg msg)
{
	 ...
	 * 			instruct ntp to update socket-corresponding 
	 * 			ntb-conn epoll state into NONE;
	...
	// 1. get the corresponding epoll_context
	epoll_context_t epoll_ctx;
	epoll_ctx = (epoll_context_t)Get(
		ntm_mgr->epoll_mgr->epoll_ctx_map, &msg.epid);
	if (epoll_ctx == NULL)
	{
	...
	if (msg.epoll_op == NTS_EPOLL_CTL_ADD || msg.epoll_op == NTS_EPOLL_CTL_MOD)
	{
		req_to_ntp_msg.ep_data = msg.ep_data;
		req_to_ntp_msg.events = msg.events;
	}
	rc = epoll_sem_shm_send(
		ntm_mgr->epoll_mgr->ntp_ep_recv_ctx, &req_to_ntp_msg);
	if (rc)
	{
...
inline void
handle_msg_nts_epoll_wait(ntm_manager_t ntm_mgr, ntm_msg msg)
{
}

inline void
handle_msg_nts_epoll_close(ntm_manager_t ntm_mgr, ntm_msg msg)
{
...
inline void
destroy_client_nt_socket_conn(nt_socket_t client_socket,
							  nts_shm_conn_t nts_shm_conn, int bound_port)
{
	...
	if (nts_shm_conn)
	{
		...
		// then Remove coresponding `nt_socket_t`
		//	in `accepted_conn_map` of `nt_listener_wrapper`
...
destroy_client_nt_socket_conn_with_sock(nt_socket_t client_socket)
{
	...
	nts_shm_conn_t nts_shm_conn;
	nts_shm_conn = (nts_shm_conn_t)Get(
		ntm_mgr->nts_ctx->nts_shm_conn_map, &client_socket->sockid);
