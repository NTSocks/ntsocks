...
static inline epoll_event_queue_t
_ep_event_queue_init(
    char *shm_addr, size_t addrlen, size_t capacity, bool is_owner)
{
    ...
    queue = (epoll_event_queue_t)
        calloc(1, sizeof(struct _epoll_event_queue));
    if (UNLIKELY(!queue))
    {
    ...
    queue->queue_bytes = sizeof(nts_event_queue) +
                         capacity * DEFAULT_EP_EVENT_QUEUE_ELE_SIZE;

    sprintf(queue->mutex_sem_name,
            "%s%s", EP_SEM_MUTEX_PREFIX, shm_addr);
    sprintf(queue->full_sem_name,
            "%s%s", EP_SEM_FULL_PREFIX, shm_addr);
    sprintf(queue->empty_sem_name,
            "%s%s", EP_SEM_EMPTY_PREFIX, shm_addr);
    ...
    if (UNLIKELY(queue->shm_fd == -1))
    {
        ...
        {
            queue->shm_fd =
                shm_open(queue->shm_addr, shm_mask, shm_mode);
            if (queue->shm_fd == -1)
            {
                perror("shm_open");
    ...
    queue->shm_queue =
        (nts_event_queue_t)mmap(NULL,
                                queue->queue_bytes, PROT_READ | PROT_WRITE,
                                MAP_SHARED, queue->shm_fd, 0);
    ...
    queue->mutex_sem = sem_open(
        queue->mutex_sem_name, O_CREAT | O_RDWR, 0666, 0);
    ...
    // counting semaphore, indicating
    //  the number of available buffers in shm ring.
    // init value = NTM_MAX_BUFS
    queue->empty_sem = sem_open(
        queue->empty_sem_name, O_CREAT | O_RDWR, 0666, queue->capacity);
    ...
    queue->full_sem = sem_open(
        queue->full_sem_name, O_CREAT | O_RDWR, 0666, 0);
    ...
    epoll_event_queue_t ctx, nts_epoll_event_int *event)
{
    ...
    memcpy((void *)&ctx->events[write_idx],
           (void *)event, DEFAULT_EP_EVENT_QUEUE_ELE_SIZE);
    __WRITE_BARRIER__;

    ctx->shm_queue->end =
        next_index(ctx->shm_queue->end, ctx->shm_queue->size);
    ...
    epoll_event_queue_t ctx, nts_epoll_event_int *event)
{
    ...
    memcpy((void *)event,
           (void *)&ctx->events[read_idx], DEFAULT_EP_EVENT_QUEUE_ELE_SIZE);
    __WRITE_BARRIER__;
    ctx->shm_queue->start =
        next_index(ctx->shm_queue->start, ctx->shm_queue->size);
    ctx->shm_queue->num_events--;
...
// Free epoll_event_queue structure,
