...
int main() {
    ...
    handle = sem_shmring_create((char *)RING_NAME, addrlen, ele_size, ele_num);
    if (!handle) 
        return -1;
    ...
    for (size_t i = 0; i < msg_cnt; i++)
    {
        ...
        retval = sem_shmring_push(handle, send_msg, strlen(send_msg));
        if (retval) {
            return -1;
        }
    ...
    handle = sem_shmring_init((char *)RING_NAME, addrlen, ele_size, ele_num);
    if (!handle) 
        return -1;
