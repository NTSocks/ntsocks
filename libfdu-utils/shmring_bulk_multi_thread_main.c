...
static void * prod(void *args) {
    ...
    for (int i = 0; i < ctx->num_msg; i+=bulk_size)
    {
        ...
        ret = shmring_push_bulk(ctx->shmring_handle, node_idxs, ele_lens, bulk_size);
        while(!ret) {
...
static void * cons(void *args) {
    ...
    for (int i = 0; i < ctx->num_msg; i+=bulk_size)
    {
        recv_cnt = shmring_pop_bulk(ctx->shmring_handle, pop_node_idxs, max_lens, bulk_size);
...
int main(int argc, char *argv[]) {
    ...
    shmring_handle = shmring_create(RING_NAME, strlen(RING_NAME), sizeof(int), capacity);
