...
int main()
{
    ...
    shmring_handle = shmring_create(
        RING_NAME, strlen(RING_NAME), sizeof(int), 1024);
    ...
    for (int i = 0; i < num_msg; i += bulk_size)
    {
        for (int j = 0; j < bulk_size; j++)
        {
            ...
            char *shm_mem = shm_offset_mem(
                mp_handler, mp_nodes[j]->node_idx);
    ...
    /* shmring */
    shmring_handle = shmring_init(
        RING_NAME, strlen(RING_NAME), sizeof(int), 1024);

    for (int i = 0; i < num_msg; i += bulk_size)
    {
        recv_cnt = shmring_pop_bulk(
            shmring_handle, pop_node_idxs, max_lens, bulk_size);
        ...
        if (recv_cnt < bulk_size)
        {
            ...
            recv_cnt = shmring_pop_bulk(shmring_handle,
                                        pop_node_idxs + recv_cnt, max_lens + recv_cnt, bulk_size - recv_cnt);
        }
