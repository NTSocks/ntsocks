...
int ntp_shm_accept(ntp_shm_context_t shm_ctx, char *shm_addr, size_t addrlen) {
    ...
    shm_ctx->node_idxs_cache = (char **) calloc(DEFAULT_MAX_BUFS, sizeof(char *));
    if (!shm_ctx->node_idxs_cache) {
        ERR("malloc shm_ctx->node_idxs_cache failed.");
        goto FAIL;
    }
    shm_ctx->node_idx_value_cache = (int *)calloc(DEFAULT_MAX_BUFS, sizeof(int));
    if (!shm_ctx->node_idx_value_cache) {
        ERR("malloc shm_ctx->node_idx_value_cache failed.");
        goto FAIL;
    }
    shm_ctx->max_lens = (size_t *) calloc(DEFAULT_MAX_BUFS, sizeof(size_t));
    if (!shm_ctx->max_lens) {
