...
{
    ntp_ntm_msg buf[NTS_MAX_BUFS + 1];
 ...
 * @return
 */
ntp_ntm_shmring_handle_t
ntp_ntm_shmring_init(char *shm_addr, size_t addrlen)
{
    assert(shm_addr);
    assert(addrlen > 0);
    ...
    shmring_handle =
        (ntp_ntm_shmring_handle_t)malloc(sizeof(ntp_ntm_shmring_t));
    ...
    int ret;
    ret = ftruncate(
        shmring_handle->shm_fd, sizeof(struct ntp_ntm_shmring_buf));
    if (ret == -1)
    {
        error("ftruncate");
        goto FAIL;
...
ntp_ntm_shmring_handle_t
ntp_ntm_get_shmring(char *shm_addr, size_t addrlen)
{
    assert(shm_addr);
    assert(addrlen > 0);
