...
int num_outstanding_reqs = 0;
int numa_node_type = 2;
int monitoring = 1;
uint64_t read_persecond[MAX_NUM_THREADS] = {0};
int running = 1;
bool to_file = false;
char file_path[256];
char file_name[128];
...
void *monitor_throughput_update(void *ptr);
...
int main(int argc, char *argv[])
{
    ...
    if (run_latency == 2 || run_latency == 3)
    {
        pthread_create(&monitor_thead, NULL,
                       monitor_throughput_update, (void *)&run_latency);
    }
...
void *monitor_throughput_update(void *ptr)
{
    ...
    uint64_t msg_cnt_start[MAX_NUM_THREADS];
    uint64_t msg_cnt_end[MAX_NUM_THREADS];
    ...
    if (*ways == 2) // test throughput in request num limit;
    {
        while (monitoring)
        {
            ...
            {
                throughput[threadId][seconds] = read_persecond[threadId];
                if (DEBUG)
                {
    ...
    else if (*ways == 3) //  test throughput in time  limit;
    {
        while (time(NULL) < tmpcal_ptr)
        {
            ...
            {
                msg_cnt_start[thr_id] = read_persecond[thr_id];
            }
            ...
            for (int thr_id = 0; thr_id < thrds; thr_id++)
            {
                msg_cnt_end[thr_id] = read_persecond[thr_id];
            }
            ...
            if (seconds < MAX_TIME_IN_BANDWIDTH)
            {
                for (int thr_id = 0; thr_id < thrds; thr_id++)
                {
                    throughput[thr_id][seconds] = 1.0 * (msg_cnt_end[thr_id] - 
                            msg_cnt_start[thr_id]) * 1000000 / (end_time - start_time);
                
                    if (DEBUG)
                    {
                        printf("thread %d throughput is %.0lf reqs/sec \n",
                                thr_id, throughput[thr_id][seconds]);
                    }
                }
            }
    ...
    memset(totalthroughput_eachsecond, 0, seconds * sizeof(double));
...
void throughput_write_with_ack_in_time(struct conn_ctx *ctx)
{
    ...
    for (size_t i = 0; i < num_outstanding_reqs; i++)
    {
        n = Nwrite(ctx->sockfd, msg, payload_size);
        if (n < 0)
        {
...
void usage(char *program)
{
    ...
    printf(" -o <outstanding requests>        the number of outstanding requests for throughput benchmark\n");
...
void parse_args(int argc, char *argv[])
{
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (strlen(argv[i]) == 2 && strcmp(argv[i], "-o") == 0)
        {
            if (i + 1 < argc)
            {
                num_outstanding_reqs = atoi(argv[i + 1]);
                if (num_outstanding_reqs < 0)
                {
                    printf("invalid num_outstanding_reqs\n");
                    exit(EXIT_FAILURE);
                }
                i++;
            }
            ...
            {
                printf("cannot read num_outstanding_reqs\n");
                usage(argv[0]);
                exit(EXIT_FAILURE);
            }
        }
