...
# Parse args
parse_arg="$1"
NTS_ROOTPATH_210=/home/ntb-server1/Yiming/NTSocks/origin-nts/
NTS_ROOTPATH_211=/home/ntb-server2/Yiming/NTSocks/origin-nts/
s_metric="$2"
s_payload_size="$3"
s_n="$4"
s_thread="$5"
...
TCP_file_path=$NTS_ROOTPATH_210/benchmark/unified_benchmark/TCP_data/BW/
...
runNTSocksBenchmark() {
    ...
    metric_arr=(1)                                                       # 0 for Latency and 1 for Throughtput (-l)
    ...
    for partition in "${partition_arr[@]}"; do
        ...
        for packet_size in "${packet_size_arr[@]}"; do
            ...
            for data_size in "${payload_size_arr[@]}"; do
                # echo "data_size = $data_size, packet_size = $packet_size"
                if [[ $data_size -ge $packet_size ]]; then

                    tmp_factor=$(( $data_size / $packet_size ))
                    # echo "[Before]  payload_size_arr[$i] = ${payload_size_arr[$i]}, tmp_factor = $tmp_factor"
                    payload_size_arr[i]=$(( ${payload_size_arr[$i]} - $tmp_factor * 8 ))
                    # echo "[After] payload_size_arr[$i] = ${payload_size_arr[$i]}"
                fi
                let i++
            done
            ...
            #             sudo sed -i "s/num_partition = ${former_partition}/num_partition = ${partition}/g" /etc/ntp.cfg
            #             cat /etc/ntp.cfg
            #             sleep 1
            ...
            for metric in "${metric_arr[@]}"; do
                for payload_size in "${payload_size_arr[@]}"; do
                    echo "payload size: $payload_size, partition: $partition, packet_size: $packet_size"
                    ...
                    #                             cd $NTS_ROOTPATH_210/benchmark/unified_benchmark/
                    #                             LD_PRELOAD=$NTS_ROOTPATH_210/libnts/build/src/lib/.libs/libnts.so ./bin/server -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 -d 10 -f $NTS_file_path &
                    #                             sleep 1
                    ...
                    #                             ssh ntb-server2@10.176.22.211 <<eeooff
                    # cd $NTS_ROOTPATH_211/benchmark/unified_benchmark/
                    # LD_PRELOAD=$NTS_ROOTPATH_211/libnts/build/src/lib/.libs/libnts.so ./bin/client -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 -d 10 &
                    # exit
                    # eeooff
...
runTCPSocketBenchmark() {
    ...
    packet_size_arr=(2048)                                        # Packet size (/etc/ntp.cfg & nts.cfg) (-c)
    metric_arr=(1)                                                # 0 for Latency and 1 for Throughtput (-l)
    thread_arr=(1 2 4 8 16 32)                                    # Thread Numbers (-t)
    n_arr=(100000)                                                # Request Numbers (-n)
                ...
                for payload_size in "${payload_size_arr[@]}"; do
                    ...
                    for thread in "${thread_arr[@]}"; do
                        ...
                        for n in "${n_arr[@]}"; do
                            ...
                            # Run 210 (Server)
                            echo "========== Running 210 NTS Application =========="
                            cd $NTS_ROOTPATH_210/benchmark/
                            ./bin/server -a 192.168.2.210 -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 1 -d 10 -f $TCP_file_path &
                            sleep 1

                            # Run 211 (Client)
                            echo "========== Running 211 NTS Application =========="
                            ssh ntb-server2@10.176.22.211 <<eeooff
cd $NTS_ROOTPATH_211/benchmark/
./bin/client -a 192.168.2.210 -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 1 -d 10 &
exit
eeooff
...
# Clean data folders
    ...
    sleep 3
    rm $NTS_ROOTPATH_210/benchmark/NTSocks_data/*
    rm $NTS_ROOTPATH_210/benchmark/TCP_data/*
    ssh ntb-server2@10.176.22.211 <<eeooff
rm $NTS_ROOTPATH_211/benchmark/NTSocks_data/*
rm $NTS_ROOTPATH_211/benchmark/TCP_data/*
exit
eeooff
...
# sleep 3
# rm $NTS_ROOTPATH_210/benchmark/temp*
