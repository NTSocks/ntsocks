...
s_partition="$6"
s_packet_size="$7"
NTS_file_path=$NTS_ROOTPATH_211/benchmark/unified_benchmark/NTS_data/LAT/
TCP_file_path=$NTS_ROOTPATH_210/benchmark/unified_benchmark/TCP_data/LAT/
...
runNTSocksBenchmark() {
    ...
    # Parse arguments
    partition_arr=(1 2 4 6 8)                                     # Partition (/etc/ntp.cfg sublink_num) (-r)
    packet_size_arr=(1024)                                        # Packet size (/etc/ntp.cfg & nts.cfg) (-c)
    payload_size_arr=(8 16 32 64 128 256 512 1024 2048 4096 8192) # Payload Size (-s), header size is 8 bytes
    metric_arr=(2)                                                # 0 for Latency and 1 for Throughtput (-l)
...
eeooff
                            ...
                            cd $NTS_ROOTPATH_210/benchmark/unified_benchmark/
                            LD_PRELOAD=$NTS_ROOTPATH_210/libnts/build/src/lib/.libs/libnts.so ./bin/server -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 &
                            sleep 1
                            ...
                            ssh ntb-server2@10.176.22.211 <<eeooff
cd $NTS_ROOTPATH_211/benchmark/unified_benchmark/
                            LD_PRELOAD=$NTS_ROOTPATH_211/libnts/build/src/lib/.libs/libnts.so ./bin/client -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 -f $NTS_file_path &
exit
eeooff
...
runTCPSocketBenchmark() {
    ...
    partition_arr=(4)                                             # Partition (/etc/ntp.cfg sublink_num) (-r)
    packet_size_arr=(1024)                                        # Packet size (/etc/ntp.cfg & nts.cfg) (-c)
    metric_arr=(2)                                                # 0 for Latency and 1 for Throughtput (-l)
                ...
                for payload_size in "${payload_size_arr[@]}"; do
                    ...
                    for thread in "${thread_arr[@]}"; do
                        ...
                        for n in "${n_arr[@]}"; do
                            ...
                            ./bin/server -a 192.168.2.210 -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 &
                            sleep 1
...
./bin/client -a 192.168.2.210 -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 -f $TCP_file_path &
exit
eeooff
