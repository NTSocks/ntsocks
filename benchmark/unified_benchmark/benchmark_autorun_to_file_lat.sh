...
s_partition="$6"
s_packet_size="$7"
NTS_file_path=$NTS_ROOTPATH_210/benchmark/unified_benchmark/NTS_data/LAT/
...
runNTSocksBenchmark() {
    ...
    packet_size_arr=(8192)                                        # Packet size (/etc/ntp.cfg & nts.cfg) (-c)
    payload_size_arr=(8 16 32 64 128 256 512 1024 2048 4096 8184) # Payload Size (-s), header size is 8 bytes
...
eeooff
                            ...
                            cd $NTS_ROOTPATH_210/benchmark/unified_benchmark/
                            LD_PRELOAD=$NTS_ROOTPATH_210/libnts/build/src/lib/.libs/libnts.so ./bin/server -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 -w -f $NTS_file_path &
                            sleep 1
                            ...
                            ssh ntb-server2@10.176.22.211 <<eeooff
cd $NTS_ROOTPATH_211/benchmark/unified_benchmark/
LD_PRELOAD=$NTS_ROOTPATH_211/libnts/build/src/lib/.libs/libnts.so ./bin/client -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 -w &
exit
eeooff
...
runTCPSocketBenchmark() {
    ...
    partition_arr=(1)                                             # Partition (/etc/ntp.cfg sublink_num) (-r)
    packet_size_arr=(2028)                                        # Packet size (/etc/ntp.cfg & nts.cfg) (-c)
    metric_arr=(0)                                                # 0 for Latency and 1 for Throughtput (-l)
                ...
                for payload_size in "${payload_size_arr[@]}"; do
                    ...
                    for thread in "${thread_arr[@]}"; do
                        ...
                        for n in "${n_arr[@]}"; do
                            ...
                            ./bin/server -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 -w -f $TCP_file_path &
                            sleep 1
...
./bin/client -p $port -l $metric -s $payload_size -n $n -t $thread -r $partition -c $packet_size -m 2 -w &
exit
eeooff
